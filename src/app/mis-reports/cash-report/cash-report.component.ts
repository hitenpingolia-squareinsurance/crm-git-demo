...
export class CashReportComponent implements OnInit {
  ...
  @ViewChild('changeStatus64', { static: false }) changeStatus64!: TemplateRef<any>;
  ...
  isSubmittedStatus:boolean = false;
  srWiseGroupData:any=[];
  constructor(
  ...
  //========= CHECK BOXES ========//
  checkUncheckAll() {
    this.dataAr.forEach(item => item.isSelected = this.masterSelected);
    this.updateCheckedList();
  }
  
  isAllSelected() {
    this.masterSelected = this.dataAr.every(item => item.isSelected);
    this.updateCheckedList();
  }
  
  updateCheckedList() {
    this.checkedList = this.dataAr.reduce((acc, { isSelected, Id }) => 
      isSelected ? [...acc, { Id }] : acc, []);
  }
  ...
  get insurerFiles(): FormArray {
    return this.statusForm.get('insurerFiles') as FormArray;
  }
  
  createFileGroup(insurerId: number, sr_ids: number[]): FormGroup {
  ...
  onStatusSelect(selected: any) {
    if (selected && selected.Id === 1) {
      this.SRWiseInsurerGroup();
    } else {
      this.srWiseGroupData = [];
      this.insurerFiles.clear();
    }
  }
  
  SRWiseInsurerGroup() {
    const selectedStatus = this.statusForm.value.UpdatedStatus;
    // Check if 'Approve' is selected
    if (!selectedStatus || selectedStatus[0].Id !== 1) {
      this.srWiseGroupData = []; // Clear variable if Cancel or deselect
      return;
    }
    this.insurerFiles.clear();
    const formData = new FormData();
    ...
    // Optional: No need to append Sr_Ids[] if not used in API
    for (let i = 0; i < this.checkedList.length; i++) {
      formData.append("Sr_Ids[]", this.checkedList[i]["Id"]);
    }
    this.api.IsLoading();
    this.api.HttpPostTypeBms("reports/CashReport/GetSRWiseInsurerGroup", formData).then(
      (result) => {
        this.api.HideLoading();
        if (result["Status"] === true) {
          this.srWiseGroupData = result["Data"];
          this.srWiseGroupData.forEach(item => {
            this.insurerFiles.push(this.createFileGroup(item.Insurance_Company_Id, item.sr_ids));
          });
          console.log("this.srWiseGroupData",this.srWiseGroupData);
        } else {
          this.api.Toast("Error", result["Message"]);
          this.srWiseGroupData = []; 
        }
  ...
  async UpdateStatus() {
    ...
    formData.append("Portal", "CRM");
    formData.append("User_Code", this.api.GetUserData("Code"));
    formData.append("Status", JSON.stringify(this.statusForm.value.UpdatedStatus));
    formData.append("Remark", this.statusForm.value.Remark);
    formData.append("checkedList", JSON.stringify(this.checkedList));
    
    const srDocsMeta: any[] = [];
    this.statusForm.get('insurerFiles')['controls'].forEach((group: any, i: number) => {
      const { insurer_id, sr_ids, file } = group.value;
      if (file) {
        formData.append(`SR_DOCS[${i}]`, file);
        srDocsMeta.push({
          insurer_id,
          sr_ids
        });
    ...
    formData.append('SR_DOCS_META', JSON.stringify(srDocsMeta)); 
    this.api.IsLoading();
    this.api.HttpPostTypeBms("reports/CashReport/UpdateStatus", formData).then(
      (result) => {
        this.api.HideLoading();
        if (result["Status"] == true) {
          this.isSubmittedStatus = false;
          this.closeUpdateStatusModel();
        } else {
          this.api.Toast("Error", result["Message"]);
        }
      },
  ...
  openUpdateStatusModal(): void {
    ...
    this.statusForm.clearValidators();
    this.dialogRef = this.dialog.open(this.changeStatus64, {
      width: '50%',
      // height: '70%',
      disableClose: true, 
    });
  ...
  onFileChange(event: any, index: number) {
    ...
    if (file) {
      ...
      if (validExtensions.includes(fileExtension)) {
        ...
        if (fileSizeInKB <= 1024 * 3) { // Check file size (3MB limit)
          this.insurerFiles.at(index).get('file').setValue(file);
          console.log('File is valid!');
        } else {
          this.api.Toast('Error', 'File size exceeds 3MB!');
          event.target.value = '';
        }
      } else {
        this.api.Toast('Error', 'Please select a valid file (PNG, JPEG, JPG, PDF)!');
        event.target.value = '';
      }
    }
  }
