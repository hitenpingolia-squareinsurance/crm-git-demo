...
export class SquareFilterComponent implements OnInit {
  ...
  LSPData: any;
  loginId: any;
  PospStatusData: { Id: string; Name: string; }[];
  Posp_Url: any;
  ...
  checkDate() {
    this.isSubmitted = true;
    var dateExport = this.SearchForm.get("DateOrDateRange").value;
    ...
    if (dateExport == '') {
      ...
      // console.log(formData);
      this.api.IsLoading();
      this.api.HttpPostType("PospManegment/exportposdata", formData).then(
        (result : any) => {
          this.api.HideLoading();
          if (result["Status"] == true) {
            this.Posp_Url =  result['url'];
            this.downloadExcelFile(result['url']);
          } else {
            this.api.Toast("Warning", result["Message"]);
          }
        },
