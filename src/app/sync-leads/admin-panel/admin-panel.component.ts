...
export class AdminPanelComponent implements OnInit {
  ...
  constructor(
    ...
    this.DataId = this.Router.url.split('/')[3];
    ...
    if(this.currentUrl == 'synced-report' || this.currentUrl == 'synced-contact' || this.currentUrl == 'synced-users'){
  ...
  ngOnInit() {
    ...
    if(this.currentUrl == 'synced-report' || this.currentUrl == 'synced-contact'){
      this.Range();
    }
  ...
  checkDataId() {
    if (this.DataId && this.DataId.substring(0, 2) === 'TN') {
  ...
  Get() {
    ...
    if (this.currentUrl == 'synced-contact') {
      apiUrl = '/SyncLeads/synced_report?User_Id=' + this.api.GetUserData("Id") + '&User_Type=' +
        this.api.GetUserType() + "&admin=" + 'contacts'  + '&sync_user_id=' +  this.DataId
    }
    if (this.currentUrl == 'synced-users') {
      apiUrl = '/SyncLeads/synced_report?User_Id=' + this.api.GetUserData("Id") + '&User_Type=' +
    ...
    if (this.currentUrl == 'synced-details') {
      apiUrl = '/SyncLeads/Transaction_Leads?User_Id=' + this.api.GetUserData("Id") + '&User_Type=' +
        this.api.GetUserType() + '&transaction=' +  this.DataId
    }
  ...
  syncContact(UserId , UserName){
    
    if (confirm("Are you sure you want to sync " +" "+ UserName +" "+ "'s  contacts!") == true) {
      alert('1');
    }
  }
