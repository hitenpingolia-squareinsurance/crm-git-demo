...
GetEmployees(e, Type): void {
  ...
  getLogs(logId: string) {
    ...
    this.api.HttpPostType("/DataDictionary/get_logs", formData).then(
      (resp: any) => {
        if (resp) {
  ...
  getLogTypeLabel(type: string): string {
    switch (type) {
      case '1': return 'Lead Added';
      case '2': return 'Lead Updated';
      case '3': return 'Lead Assigned';
      case '4': return 'Lead Re-Assigned';
      default: return type;
    }
  }
  ...
  //         that.http
  //           .post<DataTablesResponse1>(
  //             this.api.additionParmsEnc(
  //               environment.apiUrl +
  //                 "/DataDictionary/get_logs?User_Id=" +
  //                 this.api.GetUserData("Id") +
  //                 "&User_Type=" +
  //                 this.api.GetUserType() + 
  //                 "&LogId=" +
