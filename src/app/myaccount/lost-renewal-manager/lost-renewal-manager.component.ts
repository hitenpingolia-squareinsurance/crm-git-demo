...
@Component({
  selector: "app-lost-renewal-manager",
  templateUrl: "./lost-renewal-manager.component.html",
  styleUrls: ["./lost-renewal-manager.component.css"],
  changeDetection: ChangeDetectionStrategy.Default,
})
export class LostRenewalManagerComponent implements OnInit {
  ...
  TotalCaseLost: number;
  TotalCaseLostSumPremium: number;
  ...
  // TotalCaseLostSumPremium:number;
  TotalCaseRenewed:number;
  TotalCaseRenewedSumPremium:number;
  TotalCaseBase:number;
  TotalCaseBaseSumPremium:number;

  customType:any;

  showinputmobile: boolean = false;
  ...
  constructor(
    ...
    if(splitted[4] != "undefined" && splitted[4] =="old"){
      this.customType = splitted[4];
    }
  ...
  Get() {
    if (this.checkNoPageno === 1) {
      ...
      this.api
        .HttpPostType(
          "myaccount/GirdDataReportsLost?User_Id=" +
            this.api.GetUserData("Id") +
            "&User_Type=" +
            this.api.GetUserType() +
            "&PageType=Reports" +
        ...
        .then(
          (result: any) => {
            ...
            if (result["status"] == true) {
              ...
              console.log(this.dataAr ,"this.dataAr");
              ...
              if (this.pageNo <= 1) {
                ...
                this.TotalCaseLost = result["TotalCaseLost"];
                this.TotalCaseLostSumPremium =
                  result["TotalCaseLostSumPremium"];
                ...
                // this.TotalLostCase = result["TotalCaseLost"];
                // this.TotalCaseLostSumPremium = result["TotalCaseLostSumPremium"];
                this.TotalCaseRenewed = result["TotalCaseRenewed"];
                this.TotalCaseRenewedSumPremium = result["TotalCaseRenewedSumPremium"];
                this.TotalCaseBase = result["TotalCaseBase"];
                this.TotalCaseBaseSumPremium = result["TotalCaseBaseSumPremium"];
  ...
  GetBulkNewCases(AgentName: string, AgentIds: string, types = "", page) {
    // alert(123);
    ...
    const dialogRef = this.dialog.open(LostRenewalCasesComponent, {
      disableClose: true,
      data: {
        AgentName: AgentName,
        AgentId: this.AgentId,
  ...
  groupFollowupForm(Id: number, Action: string) {
    const dialogRef = this.dialog.open(
      RenewalfollowformwithagetgroupComponent,
      {
        width: "60%",
        height: "60%",
        data: { Id: Id, Status: "", Status2: "", ActionUser: "user",RenewalType:"lost"},
      }
    );
  ...
  ShowMaskingField(i) {
    ...
    if (i !== "TempMobile") {
      this.showinputmobile = true;
    }
  }
  handleMobileEnter(SrId: any, OldMobile: any, InputId: any, index: any) {
    let element = document.getElementById(InputId) as HTMLInputElement | null;
    let NewMobile;

    if (element !== null) {
      NewMobile = element.value;
    }

    const formData = new FormData();
    formData.append("Id", SrId);
    formData.append("OldMobile", OldMobile);
    formData.append("NewMobile", NewMobile);

    

    this.api.HttpPostType("Myaccount/UpdateCaseMobile", formData).then(
      (result: any) => {
        if (result["Status"] == true) {
          this.api.Toast("Success", result["Message"]);
          this.MaskingAgentMobile = "TempMobile";
          //   //   //   console.log(this.postNested);

          this.postNested[index]["CustomerMobile"] = NewMobile;
        } else {
          this.api.Toast("Error", result["Message"]);
        }
      },
  ...
  //     this.api
  //       .HttpPostType(
  //         "myaccount/GirdDataLostOldVersion?User_Id=" +
  //         this.api.GetUserData("Id") +
  //         "&User_Type=" +
  //         this.api.GetUserType() +
  //         "&PageType=Reports" +
  ...
  //               this.SharedRenewalQuery(this.RenewalQuery);
  //               this.TotalRow = result["recordsFiltered"];
  //               this.NetPremiumFilteredRecords = result["NetPremiumFilteredRecords"];
  //               this.TotalRenewalFilteredRecords = result["TotalRenewalFilteredRecords"];
  //               this.NestedPagesList = result["NestedPagesList"];
  //             }
