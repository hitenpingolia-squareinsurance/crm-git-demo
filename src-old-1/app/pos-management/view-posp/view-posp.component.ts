...
export class ViewPospComponent implements OnInit {
  ...
  UpdatePOSPTitle() {
    var Fields = this.UpdateTitle.value;
      const formData = new FormData();
      formData.append("agentId", this.setAgentKey);
      formData.append("titleval", Fields['titleUpdate']);
    
      this.api
        .HttpPostType("PospManegment/UpdatePOSPTitle", formData)
        .then(
          (result) => {
            if (result["status"] == true) {
              this.api.Toast("Success", result["msg"]);
