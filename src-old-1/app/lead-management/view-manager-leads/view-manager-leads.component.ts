...
export class ViewManagerLeadsComponent implements OnInit {
  ...
  quoteBaseUrl = 'https://www.squareinsurance.in/quotes/';
  ...
  onTimeChange(newTime: string) {
    ...
    const selectedTime = this.formatTime(newTime);
    const currentDate = new Date().toLocaleDateString("en-US");
    let fields = this.UpdateFollowForm.value;
    const selectedDate = fields["Dates"];
    const formatDate = new Date(selectedDate).toLocaleDateString("en-US");
  ...
  getDetails(Id: any) {
    ...
    this.api.HttpPostTypeBms("lms/LmsCommon/getDetailsNew", formData).then(
      (result) => {
        if (result["status"] == true) {
          this.FieldFetch = result["data"];
          console.log(this.FieldFetch.source);
  ...
  // CreateNewQuoteofflinePolicy(SrId, RegNo, ProductType, Make, Model, Variant) {
  ...
  getMotorDetails(ref_id :any){
    // alert(quotation_id);
    const formData = new FormData();
    formData.append("login_id", this.api.GetUserData("Id"));
    formData.append("login_role", this.api.GetUserType());
    formData.append("ref_id", ref_id);

    this.api.HttpPostType("DataDictionary/get_motor_details", formData).then(
      (result : any) => {
        const MotorData = result.data;

        if(MotorData.motor_type == "Two Wheeler"){
          this.Type = 1;
        }else if(MotorData.motor_type == "Private Car"){
          this.Type = 2;
        }
        ...
        document.getElementById('motormodal').click();

        if(MotorData.make !== ""){
          this.makeval = [{ Id: MotorData.make, Name: MotorData.make }];
        }else{
          this.makeval = [{ Id: "", Name: "" }];
        }
    
        if(MotorData.model !== ""){
          this.modelval = [{ Id: MotorData.model, Name: MotorData.model }];
        }else{
          this.modelval = [{ Id: "", Name: "" }];
        }
    
        if(MotorData.variant !== ""){
          this.variantval = [{ Id: MotorData.variant, Name: MotorData.variant }];
        }else{
          this.variantval = [{ Id: "", Name: "" }];
        }
        this.motorForm.patchValue({
          RegNo: MotorData.registration_no,
          SrId: MotorData.id,
          // You can also patch other values like:
          // QuoteMake: MotorData.make,
          // QuoteModel: MotorData.model,
          // QuoteFuel: MotorData.motor_type,
          // QuoteVariantValue: MotorData.variant,
          // RegYear: '', // if you have registration year
        });
        ...
        // alert(JSON.stringify(MotorData));
        // this.CreateNewQuoteofflinePolicy(MotorData["id"], MotorData["registration_no"], MotorData["motor_type"],MotorData["make"],MotorData["model"],MotorData["variant"]);
      },
      (err) => {
        const MotorData = err["error"]["text"];
        // console.log(this.dataArr);
      }
    );
  }

  openquote(quotationId: any): void {
    if (!quotationId) {
      console.warn('Invalid quotationId');
      return;
    }
  
    // Base64 encode the ID (you can replace this with custom encryption)
    const encodedId = btoa(quotationId.toString());
  
    // Combine with base URL
    const fullUrl = this.quoteBaseUrl + encodedId;
  ...
  GetVariant(SearchTerm: any) {
    ...
    const SearchMa = this.makeval;
    const SearchMo = this.modelval;
    const SearchFu = this.motorForm.get("QuoteFuel").value;
    var SearchTerm1 = "";
  ...
  UpdateMotorForm() {
    this.motorSubmitted = true;
    if (this.motorForm.invalid) {
      return;
    } else {
      const formData = new FormData();
      var Fields = this.motorForm.value;
      ...
      formData.append("SrId", Fields["SrId"]);
      formData.append("login_id", this.api.GetUserData("Id"));
      formData.append("login_type", this.api.GetUserType());
      formData.append("Reg_Year", Fields["RegYear"]);

      this.api.IsLoading();
      this.api
        .HttpPostType(
          "/Prequotes/CreateOnlineMotorQuote",
          formData
        )
        .then(
          (result) => {
