...
export class ViewEmployeeComponent implements OnInit {
  ...
  constructor(
    ...
    this.IPForm = this.formBuilder.group({
      ipArray: this.formBuilder.array([this.createIpField()])
    });
  ...
  get ipArray(): FormArray {
    return this.IPForm.get('ipArray') as FormArray;
  }


  addIPField() {
    this.ipArray.push(this.createIpField());
  }
  
  removeIPField(index: number) {
  ...
  AddIP(dataId: any) {
    ...
    this.api.HttpPostType("b-crm/Employee/getIP", formData).then(
      (result: any) => {
        this.api.HideLoading();
        if (result["status"] == true) {
          // this.api.Toast("Success", result["msg"]);
  
          // Reset form and form array
          this.IPForm.reset();
          this.ipArray.clear();
  
          // Patch IP values to ipArray
          const ipList = result["data"];
          ipList.forEach((ip: string) => {
            this.ipArray.push(this.formBuilder.control(ip, [
              Validators.required,
              Validators.pattern(
                /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/
              )
          ...
          // If no IPs, add at least one field
          if (ipList.length === 0) {
            this.addIPField();
          }
        }
  ...
  ResetIPForm(){
    ...
    this.IPForm.reset();  // Resets form values
    this.ipArray.clear();  // Clears the form array (removes all IP fields)
    this.ipArray.push(this.formBuilder.control('', [Validators.required, Validators.pattern(
      /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/
    )])); // Re-add the initial IP input field
  ...
  IPFormSubmit() {
    this.isSubmittedip = true;
  
    if (this.IPForm.valid) {
      const formData = new FormData();
      const ipList = this.IPForm.value.ipArray;
  
      // Append each IP as ip[]
      ipList.forEach((ip: string, index: number) => {
      ...
      this.api.HttpPostType("b-crm/Employee/StoreipData", formData).then(
        (result: any) => {
          this.api.HideLoading();
          if (result["status"] == true) {
            this.api.Toast("Success", result["msg"]);
            ...
            const close = document.getElementById("closeFormModelIp");
            close.click(); // Safe close
            this.Reload();
          } else {
            this.api.Toast("Warning", result["msg"]);
          }
      ...
      console.log(this.IPForm.controls);
      return;
    }
  }
